...
const { extractCotacao } = require('./api.bcb')
...
test('getUrl', () => {
  const url = api.getUrl('MINHA_DATA')
  expect(url).toBe('https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/odata/CotacaoDolarDia(dataCotacao=@dataCotacao)?@dataCotacao=%27MINHA_DATA%27&$top=100&$format=json')
})
